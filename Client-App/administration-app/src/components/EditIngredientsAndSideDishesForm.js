...
const EditIngredientsAndSideDishesForm = ({ hidden }) => {
	...
	const handleBackClick = () => {
		dispatch({ type: productConstants.UPDATE_PRODUCT_BACK_TO_FIRST_PAGE });
	};

	const handleSubmit = () => {
		let product = { ...productState.updateProduct.product };
	...
	useEffect(() => {
		setIngredients(productState.updateProduct.product.EntityDTO.Ingredients);
		setSideDishes(productState.updateProduct.product.EntityDTO.SideDishes);
	}, [productState.updateProduct.product.EntityDTO]);

	return (
		<div hidden={hidden}>
			<div className="row">
				<div className="col-md-6 grid-margin stretch-card w-100">
...
export default EditIngredientsAndSideDishesForm;
