		...
		<TouchableOpacity onPress={() => handlePressOnOrder(item)} style={orderHistoryStyles.itemContainer}>
			<View style={orderHistoryStyles.itemSubContainer}>
				<View style={orderHistoryStyles.content}>
					<Image source={item.OrderType === "DELIVERY" ? icons.delivery : icons.restaurantTable} resizeMode="contain" style={orderHistoryStyles.image} />
					<View style={orderHistoryStyles.infoStyle}>
						...
						<Text style={orderHistoryStyles.price}>{Number(item.Price).toFixed(2)} RSD </Text>
						<Text style={orderHistoryStyles.date}>{Moment(item.CreatedDate).format("DD MMM yyyy hh:mm")}</Text>
					</View>
				</View>
			</View>
		</TouchableOpacity>
