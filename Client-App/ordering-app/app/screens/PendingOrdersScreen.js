...
function PendingOrdersScreen({ navigation }) {
	...
	const handleRejectOrder = (orderId) => {
		Alert.alert("Confirm reject", "Are you sure to reject order?", [
			{
				text: "Yes",
		...
		<View style={pendingOrdersStyles.itemContainer}>
			<View style={pendingOrdersStyles.itemSubContainer}>
				<View style={pendingOrdersStyles.content}>
					<View>
						<Image source={item.OrderType === "DELIVERY" ? icons.delivery : icons.restaurantTable} resizeMode="contain" style={pendingOrdersStyles.image} />
							...
							<Image
								...
								style={
									item.OrderStatus === "UNCONFIRMED"
										? pendingOrdersStyles.cicrleIconStyleUnconfirmed
										: {
												...(item.OrderStatus === "CONFIRMED"
													? pendingOrdersStyles.cicrleIconStyleConfirmed
													: { ...(item.OrderStatus === "READY" ? pendingOrdersStyles.cicrleIconStyleReady : pendingOrdersStyles.cicrleIconStyleDelivering) }),
										  }
								}
							/>
					...
					<View style={pendingOrdersStyles.infoStyle}>
						...
						<Text style={pendingOrdersStyles.date}>{Moment(item.CreatedDate).format("DD MMM yyyy HH:mm")}</Text>
						{item.OrderType === "DELIVERY" ? <Text style={pendingOrdersStyles.address}>{item.Address}</Text> : <View></View>}

						<Text style={pendingOrdersStyles.price}>{Number(item.Price).toFixed(2)} RSD</Text>
					</View>
				</View>
				{item.OrderStatus != "DELIVERING" ? (
					<TouchableOpacity onPress={() => handleRejectOrder(item.Id)}>
						<Image source={icons.remove} resizeMode="contain" style={pendingOrdersStyles.rejectIcon} />
					</TouchableOpacity>
				) : (
					<View></View>
				)}
		...
		<SafeAreaView style={{ flex: 1, backgroundColor: "white" }}>
			{orderState.pagesError.pendingOrdersError === false ? (
				...
				<View style={pendingOrdersStyles.errorContainer}>
					<Text style={pendingOrdersStyles.errorMessage}>We have some problem. Please try again!</Text>
					<Text style={pendingOrdersStyles.reloadPageMessage}>Reload page</Text>
					<TouchableOpacity onPress={() => setIsFetching(true)}>
						<Image source={icons.refreshPage} resizeMode="contain" style={pendingOrdersStyles.refreshPageImage} />
					</TouchableOpacity>
				</View>
			)}
		</SafeAreaView>
